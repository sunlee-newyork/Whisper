...
var incomingDiv = "<div id='whisper_incoming'></div>";
$('body').append(incomingDiv);
  ...
  on_message: function (message) {
    ...
    if (Whisper.fadeout !== null) {
      var fadeout = setTimeout(function() {
        $("#whisper_incoming").fadeOut('slow');
      }, Whisper.fadeout);  
    }

    $('#whisper_incoming').click(function() {
      if (fadeout) {
        clearTimeout(fadeout);  
      }
      // Attempt at detecting if #whisper_incoming is clicked within 3 seconds
      $(document).on('click', function(event) { 
        if($(event.target).parents().index($('#whisper_incoming')) == -1) {
          if($('#whisper_incoming').is(":visible")) {
            $('#whisper_incoming').fadeOut('slow');
          }
        }
      });
